...
class SAM2Train(SAM2Base):
    def __init__(
        ...
        # method for point sampling during training
        # "uniform" (sample uniformly from error region) or "center" (use the point with the largest distance to error region boundary)
        # default to "uniform" for training, but can be set to "center" to match evaluation strategy
        ...
        freeze_obj_ptr_proj=False,
        freeze_obj_ptr_tpos_proj=False,
        **kwargs,
    ):
        ...
        if freeze_obj_ptr_proj:
            for p in self.obj_ptr_proj.parameters():
                p.requires_grad = False

        if freeze_obj_ptr_tpos_proj:
            for p in self.obj_ptr_tpos_proj.parameters():
                p.requires_grad = False
    ...
    def prepare_prompt_inputs(self, backbone_out, input, start_frame_idx=0):
        ...
        if self.num_correction_pt_per_frame == 0:
            frames_to_add_correction_pt = []
    ...
    def _iter_correct_pt_sampling(
        ...
        for _ in range(self.num_correction_pt_per_frame):
            ...
            new_points, new_labels = get_next_point(
                ...
                method=self.pt_sampling_for_train
                if self.training
                else self.pt_sampling_for_eval,
            )
